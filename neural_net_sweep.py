...
def train():
    ...
    # Ensure we always have at least two layers to log
    

    run_name = f"hl_{config.architecture['num_of_hidden_layers']}_bs_{config.batch_size}_ac_{config.activation_function}"
...
if __name__ == '__main__':
	...
	sweep_config = {
	    ...
	    'parameters': {
	        ...
	        'architecture': {
	            'values': [
	                {'num_of_hidden_layers': 3, 'hidden_layer_size': [32, 64, 10]},
	                {'num_of_hidden_layers': 4, 'hidden_layer_size': [32, 64, 128, 10]},
	                {'num_of_hidden_layers': 5, 'hidden_layer_size': [32, 64, 128, 356, 10]},
	            ]
	        },
	...
	(train_data,train_label),(test_data,test_label) = load_dataset('fashion_mnist')
	train_data,train_label,test_data,test_label,val_data,val_label = format_data(train_data,train_label,test_data,test_label)
	sweep_id = sweep_id = wandb.sweep(sweep_config,project='dl-assignment1')
